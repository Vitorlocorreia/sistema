...
// Skip index 0 (already inserted in test call)
const sqlTuples = data.slice(1).map((row) => {
  ...
  // Regra do cliente: > 30.000 entra como 'Bloqueado', <= 30.000 entra como 'Lançado'
...
const CHUNK_SIZE = 35;
const sqlHeader = `INSERT INTO public.contas (
  empresa_id,
  obra_id,
  fornecedor_id,
  tipo,
...
const chunkFiles = [];
for (let i = 0; i < sqlTuples.length; i += CHUNK_SIZE) {
  const chunkTuples = sqlTuples.slice(i, i + CHUNK_SIZE);
  const sql = sqlHeader + chunkTuples.join(',\n') + ';';
  const fileName = `batch_pos_${Math.floor(i / CHUNK_SIZE)}.sql`;
  fs.writeFileSync(path.join(__dirname, fileName), sql);
  chunkFiles.push(fileName);
}

console.log(`Generated ${chunkFiles.length} batch SQL files:`, chunkFiles);
