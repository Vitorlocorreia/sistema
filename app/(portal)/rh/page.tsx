...
export default function RhPage() {
  ...
  return (
    <>
      ...
      {convites.length > 0 && <div style={{ display: 'grid', gridTemplateColumns: 'minmax(230px,0.8fr) minmax(0,2fr)', gap: 14, marginBottom: 14, alignItems: 'start' }}>
        <section style={card}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}><strong style={{ fontSize: 12 }}>Em cadastro</strong><span style={{ color: C.inkSoft, fontSize: 10 }}>{convites.length}</span></div><div style={{ display: 'grid', gap: 6 }}>{convites.map(invite => { const expired = new Date(invite.expires_at).getTime() <= Date.now() && ['ativo', 'em_preenchimento'].includes(invite.status); const label = invite.status === 'devolvido' ? 'Devolvido' : invite.status === 'revogado' ? 'Revogado' : expired ? 'Expirado' : invite.status === 'aguardando_aprovacao' ? 'Aguardando aprovação' : invite.status === 'em_preenchimento' ? `Etapa ${invite.etapa_atual}/4` : 'Link gerado'; return <button key={invite.id} onClick={() => setSelectedInvite(invite)} style={{ textAlign: 'left', padding: 10, background: selectedInvite?.id === invite.id ? '#F59E0B18' : '#0B0C0E', color: C.ink, border: `1px solid ${selectedInvite?.id === invite.id ? '#F59E0B66' : C.border}`, borderRadius: 5, cursor: 'pointer' }}><strong style={{ fontSize: 11 }}>{invite.nome_destinatario}</strong><div style={{ color: C.inkSoft, fontSize: 9, marginTop: 4 }}>{invite.cargo || 'Cargo não informado'}</div><div style={{ color: invite.status === 'devolvido' || expired ? '#F87171' : C.amber, fontSize: 9, fontWeight: 800, marginTop: 5 }}>{label}</div>{invite.status === 'devolvido' && invite.justificativa_devolucao && <div style={{ color: '#FCA5A5', fontSize: 9, marginTop: 4 }}>{invite.justificativa_devolucao}</div>}<div style={{ color: C.inkSoft, fontSize: 8, marginTop: 5 }}>{invite.token_code ? `Código: ${invite.token_code}` : 'Código ainda não gerado'}</div></button> })}</div></section>
