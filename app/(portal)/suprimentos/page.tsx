...
export default function QuadrosPage() {
  ...
  async function createBoard() {
    let nome = await prompt('Novo quadro', { description: 'Nome do novo quadro:' })
    if (nome === null) return
    nome = nome.trim()
    if (!nome) return
  ...
  async function renameBoard() {
    ...
    let nome = await prompt('Renomear quadro', { description: 'Novo nome do quadro:', defaultValue: selectedBoard.nome })
    if (nome === null) return
    nome = nome.trim()
    if (!nome || nome === selectedBoard.nome) return
  ...
  async function addColumn() {
    if (!boardId) return
    let titulo = await prompt('Nova coluna', { description: 'Título da coluna:' })
  ...
  async function editColumn(col: QuadroColuna) {
    let titulo = await prompt('Renomear coluna', { description: 'Título da coluna:', defaultValue: col.titulo })
    if (titulo === null) return
    titulo = titulo.trim()
    if (!titulo || titulo === col.titulo) return
  ...
  async function addCustomField() {
    if (!boardId) return
    let nome = await prompt('Campo personalizado', { description: 'Nome do campo personalizado:' })
    if (nome === null) return
    nome = nome.trim()
    if (!nome) return
  ...
  async function addAutomation() {
    if (!boardId) return
    let nome = await prompt('Nova automação', { description: 'Nome da automação:', defaultValue: 'Ao mover para concluído' })
    if (nome === null) return
    nome = nome.trim()
    if (!nome) return
      ...
      <div style={{ display: 'flex', gap: 14, overflowX: 'auto', paddingBottom: 18, alignItems: 'flex-start' }}>
        {columns.map((col, index) => <section key={col.id} onDragOver={e => e.preventDefault()} onDrop={() => void dropCard(col.id)} style={{ width: 300, minWidth: 300, background: '#11131A', border: `1px solid ${C.border}`, borderTop: `3px solid ${col.cor}`, borderRadius: 5, padding: 11 }}>
          ...
          <div style={{ display: 'grid', gap: 9, minHeight: 70 }}>{cardsByColumn[col.id]?.map(card => <article key={card.id} draggable onDragStart={e => { e.stopPropagation(); setDraggingCard(card.id) }} onDragEnd={() => setDraggingCard(null)} onClick={() => setDraft({ id: card.id, coluna_id: card.coluna_id, titulo: card.titulo, descricao: card.descricao || '', responsavel: card.responsavel || '', prioridade: card.prioridade, prazo: card.prazo || '', etiquetas: card.etiquetas.join(', '), anexos: card.anexos || [] })} style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderLeft: `3px solid ${priorityColor[card.prioridade]}`, borderRadius: 4, padding: 11, cursor: 'grab', opacity: draggingCard === card.id ? .55 : 1 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>{(Array.isArray(card.etiquetas) ? card.etiquetas : (card.etiquetas ? String(card.etiquetas).split(',') : [])).map(tag => <span key={tag} style={{ fontSize: 8, background: '#F59E0B18', color: C.amber, padding: '2px 5px' }}>{String(tag).trim()}</span>)}</div>
