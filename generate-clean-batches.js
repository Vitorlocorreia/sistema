...
const sqlTuples = data.map((row, idx) => {
...
console.log(`Total tuples generated: ${sqlTuples.length}`);
...
for (let i = 0; i < sqlTuples.length; i += CHUNK_SIZE) {
  ...
  fs.writeFileSync(path.join(__dirname, `clean_batch_${Math.floor(i / CHUNK_SIZE)}.sql`), sql);
}

console.log('Clean batches created!');
